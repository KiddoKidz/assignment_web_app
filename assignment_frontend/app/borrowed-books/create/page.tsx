...
import { createBorrowedBook } from '@/services/borrowedBook.service'
...
export default function CreateBorrowedBookPage() {
    ...
    const createMutation = useMutation({
        mutationFn: (data: BorrowedBookInput) => createBorrowedBook(data),
        onSuccess: () => {
            toast.success("BorrowedBook created successfully")
            queryClient.invalidateQueries({ queryKey: borrowedBookKeys.lists() })
            router.push("/borrowed-books")
        },
        onError: () => {
            toast.error("Failed to create borrowedBook")
        },
    })
    ...
    return (
        <div className="flex flex-1 flex-col gap-4 p-4">
            <h1 className="text-3xl font-bold tracking-tight">Create Borrowed Book</h1>
            <Card className="w-full">
                ...
                <CardFooter className="flex justify-end gap-3 pt-2 sm:flex-row">
                    ...
                    <Button
                        ...
                        className="w-full sm:w-auto"
                    >
                        {isSubmitting || createMutation.isPending ? "Creating..." : "Create Borrowed Book"}
                    </Button>
                </CardFooter>
            </Card>
        </div>
